...
class Viewer():
    ...
    def __init__(self,wname="Viewer",size=[1200,800]) :
        ...
        self.color = np.empty([0,3],dtype=np.float32)
    ...
    def add_patch(self,X,Y,Z,wireframe=False,color=[0.5,0.5,0.5]) :
        ...
        # add the mesh
        self.add_mesh(V,F,E,wireframe,color)
        
        
    def add_mesh(self,V,F,E=None,wireframe=False,color=[0.5,0.5,0.5]) :
    ...
    def render(self) :
        ...
        self.location.color = glGetUniformLocation( program, 'incolor')
        ...
        while not glfw.window_should_close(window) and glfw.get_key(window,glfw.KEY_ESCAPE) != glfw.PRESS:    
            ...
            glUniform3fv( self.location.color, 1, self.color )
